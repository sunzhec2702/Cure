...
static char log_buf[MAX_LOG_MESSAGE_LEN];
...
static u32 time_to_seconds(calendar_obj *cal)
{
    return cal->hour * 3600 + cal->min * 60 + cal->sec;
}

static ErrorStatus sync_daily_record()
{
    u32 byte_write;
...
static ErrorStatus sync_total_record()
{
    u32 byte_write;
...
static ErrorStatus update_record_file()
{
...
ErrorStatus log_system_init()
{
    ...
    {
        f_lseek(&total_record_file, 0);
        ret = f_read(&total_record_file, &total_use_time, sizeof(u32), &byte_read);
        if (ret != FR_OK)
            return ERROR;
        sync_total_record();
    }
...
ErrorStatus record_update_output_status(output_status status)
{
    calendar_obj *cur_cal = get_current_calendar();
    switch(status)
    {
    case OUTPUT_ENABLE:
        ...
        single_use_time.start_time = time_to_seconds(cur_cal);
    ...
    case OUTPUT_DISABLE:
        ...
        single_use_time.end_time = time_to_seconds(cur_cal);
        break;
    default:
        return ERROR;
    }
...
ErrorStatus record_update_output_log()
{
    u8 ret = 0;
    u32 byte_write;
    memset(log_buf, 0, MAX_LOG_MESSAGE_LEN);
    if (is_calendar_initialized() == FALSE)
        return ERROR;
    calendar_obj *cur_cal = get_current_calendar();
    ...
    if (f_lseek(&log_file, log_file.fsize) != FR_OK)
        return ERROR;
    snprintf(log_buf, MAX_LOG_MESSAGE_LEN, "%d-%d-%d: status %d\n", cur_cal->hour, cur_cal->min, cur_cal->sec, current_status);
    f_write(&log_file, log_buf, strlen(log_buf), &byte_write);
    /*
    ret = f_printf(&log_file, "%d-%d-%d\n", cur_cal->hour, cur_cal->min, cur_cal->sec);
    if (ret != FR_OK)
        return ERROR;
    */
    /*
    if (f_printf(&log_file, "%d-%d-%d\n", cur_cal->hour, cur_cal->min, cur_cal->sec) != FR_OK)
        return ERROR;
    */
    /*
    ret = f_printf(&log_file, "current output status is %d\n", current_status);
    if (ret != FR_OK)
        return ERROR;
    */
    /*
    if (f_printf(&log_file, "current output status is %d\n", current_status) != FR_OK)
        return ERROR;
    */
